...
if (!process.env.SEGS_URL) {
	logger.error(
		"SEGS_URL not set, please set it in your environment variables.",
	);
	process.exit(1);
}
const SEGS_URL = process.env.SEGS_URL;
logger.debug("SEGS_URL: %s", SEGS_URL);
const result = await facebook.story.getVideosAndAudioUrls(
	await getPage(),
	facebook.getRandomCookie(),
	SEGS_URL,
	"html"
);
logger.debug("result: %o", result);
const videoAndAudios = await facebook.story.getVideosAndAudioUrls(
	await getPage(),
	facebook.getRandomCookie(),
	SEGS_URL,
	"interception"
